const timeEstimation = (date: string | number | Date) => {
  ...
  if (anos > 0 || meses > 1) return `Más de un mes`;
  else if (meses == 1)
    return `Un mes${dias == 0 ? "" : ` y ${dias == 1 ? "día" : "dias"}`}`;
  else if (dias > 0) return `Hace ${dias} ${dias == 1 ? "día" : "dias"}`;
  else if (horas < 1 && minutos < 1) return `Hace un momento`;
  else if (horas < 1)
    return `Hace ${minutos} ${minutos == 1 ? "minuto" : "minutos"}`;
  else
    return `${horas} horas, y ${minutos} ${
