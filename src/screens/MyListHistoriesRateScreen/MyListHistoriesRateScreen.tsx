...
const MyListHistoriesRateScreen = ({ navigation }: { navigation: any }) => {
    ...
    <ScrollView contentContainerStyle={styles.scrollView}>
      ...
      >
        <Text style={{ fontWeight: "bold", fontSize: 20, marginBottom: 30 }}>
          Listado de mis recomendaciones
        </Text>
        <View
          style={{
            flex: 1,
        ...
        >
          <View style={{ gap: 10, width: "100%", paddingBottom: 20 }}>
            {historiesMocks.histories.map((item: IHistory, i: number) => {
              return (
                <CardElementHistory
                  key={i}
                  history={item}
...
export default MyListHistoriesRateScreen;
